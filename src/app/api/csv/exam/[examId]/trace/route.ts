...
import { buildExamTraceCsv, buildExamTraceCsvFromSnapshot } from "@/lib/pdf/exam-csv";
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ examId: string }> }) {
  ...
  const csv = version
    ? buildExamTraceCsvFromSnapshot(version.snapshot)
    : buildExamTraceCsv(exam);
  const title = version?.snapshot.title ?? exam.title;
  const safeName = title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || `prova-${exam.id}`;
