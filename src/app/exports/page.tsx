          ...
          <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: "1.5rem", alignItems: "start", marginBottom: "2rem" }}>
            ...
            {selectedExam && (
              <div>
                ...
                <GabaritoUpload key={selectedExam.id} examId={selectedExam.id} isNew={isNew} />

                <div className="card" style={{ marginBottom: "1.25rem" }}>
                  <h3 style={{ fontWeight: 600, marginBottom: "0.75rem" }}>PDF Completo</h3>
                  <p style={{ fontSize: "0.875rem", color: "var(--muted)", marginBottom: "1rem" }}>
                    Todos os sets ({selectedExam.sets.map((s) => `Set ${s.label}`).join(", ")}) + gabarito na última página.
                  </p>
                  <a href={`/api/pdf/exam/${selectedExam.id}`} className="btn btn-primary" download>
                    ⬇ Baixar PDF da Prova
                  </a>
                </div>
          ...
          {allExamQuestions.length > 0 && (
            <div className="card">
              ...
              <p style={{ fontSize: "0.8rem", color: "var(--muted)", marginBottom: "1.5rem" }}>
                {allExamQuestions.length} questão(ões) únicas utilizadas em provas — com alternativas originais, resposta correta e justificativa.
              </p>
